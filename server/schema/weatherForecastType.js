...
const weatherForecastType = new GraphQLObjectType({
    ...
    fields: () => ({
        ...
        temp_f_data: {
            type: new GraphQLList(GraphQLFloat),
            resolve: (obj) => {
                return obj.list.map(weather => to_fahrenheit(weather.main.temp));
            }
        },
    })
});

const to_fahrenheit = (temp) => {
    return ((temp * 9 / 5) - 459.67).toFixed(2);
};

const to_celsius = (temp) => {
...
const mainType = new GraphQLObjectType({
    ...
    fields: () => ({
        ...
        temp_f: {
            type: GraphQLFloat,
            resolve: (obj) => {
                return to_fahrenheit(obj.temp);
            }
        },
        temp_c: {
            type: GraphQLFloat,
            resolve: (obj) => {
                return to_celsius(obj.temp);
            }
        },
    }),
});
